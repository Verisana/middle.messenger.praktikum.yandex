...
export function getSelectedSideChat(
  ...
  if (selected.length > 1) {
    throw new Error("Received two or more selected objects. Something is wrong")
    ...
    return {
      ...
      last_message: {
        ...
        time:
          selected[0].props.Message.props.Time?.props.timeMachine === undefined
            ? ""
            : selected[0].props.Message.props.Time?.props.timeMachine,
        timeHuman:
          selected[0].props.Message.props.Time?.props.timeHuman === undefined
            ? ""
...
export class SideChat extends Block<ISideChatProps> {
  constructor(params: BlockParams<ISideChatProps>) {
    const { props } = params
    const selected = store.select("selectedChat") as IChatsResponse | undefined
